...
/*
 * this is the old version of car list that works on api 8 and 10 but not 11 and newer.
 */

public class CarListOld extends ListActivity {
	
	Car car1 = new Car(1,70,216,39,"40 CHEVY BUSINESS COUPE","PRIMERED BODY IS COMPLETE AND SOLID - NEEDS ONLY PAINT","SMOKES SOME AND HAS LOUD EXHAUST THAT COVERS ROD KNOCKS - 'engine needs work'.",2865,2.3,5,0,0,2.94,1.68,1,4.11,"CHEV",4,40,"");//from JUNK0CAR.DG1
	...
	Car car4 = new Car(4,0,248,29,"42 PONTIAC 'TORPEDO' COUPE","THE CAR IS NO LONGER RUNNING - THE OLD STRAIGHT 8 LAYS DEAD UNDER THE HOOD.","OLDS GRILLE - NO PONTIAC STRIPES - GOOD PAINT WITH FLAMES",3320,2.3,5,2,0,2.94,1.68,1,4.55,"PONT",6,42,"");//is car #6 in file, others are still 1-3
    		...
    		chooseCar.setPositiveButton("Yes", new DialogInterface.OnClickListener() {// ************** if player clicked "yes"
    			public void onClick(DialogInterface d, int i) {
    				...
    				if (counter == 3) {
    					...
    					Intent intent = new Intent(CarListOld.this, CarFix.class);
	...
	public void fillData() {
		...
		//Log.i(car3.desc1, money[counter]);
		values = new String[] {carS3, money[counter]};
		//values = new String[] {carS1, carS2, carS3, carS4, money[counter]};
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(CarListOld.this, R.layout.carrow, R.id.eachcar, values);
		//ArrayAdapter<String> adapter = new ArrayAdapter<String>(CarList.this, R.id.eachcar);
		//adapter.add(carS3);
		//adapter.add(money[counter]);
